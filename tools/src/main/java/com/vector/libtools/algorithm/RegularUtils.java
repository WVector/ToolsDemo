...
public class RegularUtils {
    ...
    //抓取网页img的地址
    public static final String IMG_PATTERN = "<(?:img|IMG)\\s+(?:[^>]*)\\s+(?:src|SRC)\\s*=\\s*([^>\\s]+)[^>]+>";
    public static final String IMG_PATTERN_REPLACE = "<p align=\"center\"><img src=$1 width=\"100%\" !important;></p>";
    //修改字体
    public static final String FONT_PATTERN = "(?:font-size|FONT-SIZE)\\s*:\\s*\\d{1,}\\s*(?:px|pt|PX|PT)";
    public static final String FONT_PATTERN_REPLACE = "font-size:11pt";

    /**
     * @param html 网页
     * @return 网页中所有的img地址
     */
    public static ArrayList<String> getImgUrls(String html) {
        ArrayList<String> imgUrls = new ArrayList<>();
        Pattern pattern = Pattern.compile(IMG_PATTERN, Pattern.MULTILINE | Pattern.CASE_INSENSITIVE);
        Matcher matcher = pattern.matcher(html);

        while (matcher.find()) {
            imgUrls.add(filterUrl(matcher.group(1)));
        }
    ...
    private static String filterUrl(String imgUrl) {
        if (imgUrl.startsWith("\"") && imgUrl.endsWith("\"")) {
            return imgUrl.substring(imgUrl.indexOf("\"") + 1, imgUrl.lastIndexOf("\""));
        } else if (imgUrl.startsWith("'") && imgUrl.endsWith("'")) {
            return imgUrl.substring(imgUrl.indexOf("'") + 1, imgUrl.lastIndexOf("'"));
        } else {
            return imgUrl;
        }
